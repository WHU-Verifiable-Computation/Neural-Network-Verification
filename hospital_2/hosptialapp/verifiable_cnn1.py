...
class Conv1d_numpy:
    ...
    def infer(self, inputs):
        ...
        for w in range(output_w):   #不包括output_w，相当于input_w为奇数的话忽略最后一列
            ...
            output_c = package(output_y0,output_y1,output_t)
            res = conv_verify(output_c,input_t,coefficient)
...
class Avgpool_numpy:
    ...
    def infer(self,inputs):
        ...
        for w in range(output_w):   #不包括output_w，相当于input_w为奇数的话忽略最后一列
            ...
            output_y0 = output[0::3,:]
            output_y0 = output_y0.reshape(batch_size1,-1)
            output_y0 = encode(output_y0)

            output_y1 = output[1::3,:]
            output_y1 = output_y1.reshape(batch_size1,-1)
            output_y1 = encode(output_y1)

            output_t = output[2::3,:]
            output_t = output_t.reshape(batch_size1,-1)
            output_t = encode(output_t)

            output_c = package(output_y0,output_y1,output_t)
            res = conv_verify(output_c,input_t)
            res = res.reshape(batch_size1,input_channel)
            verify_res[:,:,w]=res
